...
class FlarumEventSubscriber implements ShouldQueue
{
    ...
    {
        $flarumUserId = $event->flarumUserId;
        $method = 'PATCH';
        $endpoint = "/api/users/{$flarumUserId}";
    ...
    public function onUserLogin($event)
    {
        ...
        $this->setRememberMeSession($token);
    }
    ...
    /**
     * Set RememberMeSession with returned token for Flarum access
     */
    private function setRememberMeSession($token)
    {
        session([self::REMEMBER_ME_KEY=>$token]);
    }
    /*
    private function setRememberMeCookie($token)
    {
        $this->setCookie(self::REMEMBER_ME_KEY, $token, time());
    }
